...
try:
    from doc2vec_inner import train_document_dbow, train_document_dm, train_document_dm_concat
    ...
    def train_document_dbow_neighbors(model, doc_words, doctag_indexes, alpha, beta, work=None,
                            train_words=False, learn_doctags=True, learn_words=True, learn_hidden=True,
                            word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None):
        """
        ...
        """
        assert model.doc_label is not None, 'there is no label'
        if doctag_vectors is None:
            doctag_vectors = model.docvecs.doctag_syn0
        if doctag_locks is None:
            doctag_locks = model.docvecs.doctag_syn0_lockf
...
def train_document_dbow_neighbors(model, doc_words, doctag_indexes, alpha, beta, work=None,
    ...
    for doctag_index in doctag_indexes:
        ...
        if model.learn_unlabel:    
            train_doc_neighbors_random(model,doctag_index, beta, learn_vectors=learn_doctags, learn_hidden=learn_hidden)
        else:
            train_doc_neighbors(model,doctag_index, beta, learn_vectors=learn_doctags, learn_hidden=learn_hidden)
    return len(doc_words)
            
def train_doc_neighbors_random(model,doctag_indexes, beta, learn_vectors=True, learn_hidden=True,
    ...
    lock_factor = doctag_locks[doctag_indexes]
    ...
    nbs_labels = np.array(model.doc_label[nbs_indexes],copy = True)
    #positive ratio!
    pos_prob = sum(nbs_labels ==1)/model.num_neighbor
    if base_label == 0:
        lij = np.zeros(model.num_neighbor)        
        temp = random.random()
        if temp > pos_prob:
            lij = np.ones(model.num_neighbor) *base_label
        else:
            lij = (-np.ones(model.num_neighbor))*base_label
    else:
...
def train_doc_neighbors(model,doctag_indexes, beta, learn_vectors=True, learn_hidden=True,
    ...
    nbs_indexes, nbs_vectors, nbs_locks, ignored = indexed_doctags    
    """    
    syn0norm = empty(model.docvecs.doctag_syn0.shape, dtype = REAL)
    np_divide(model.docvecs.doctag_syn0, sqrt((model.docvecs.doctag_syn0**2).sum(-1))[...,newaxis], syn0norm)
    nbs_indexes = model.find_nn_idx(doctag_indexes,syn0norm,model.num_neighbor)
...
class Doc2Vec(Word2Vec):
    ...
    def __init__(self, documents=None, doc_label=None, size=300, alpha=0.025, window=8, min_count=5, beta = 0.010,
                 max_vocab_size=None, sample=0, seed=1, workers=1, min_alpha=0.0001, learn_unlabel = 0, dbow_neighbor = 0,
                 dm=1, hs=1, negative=0, dbow_words=0, dm_mean=0, dm_concat=0, dm_tag_count=1, num_neighbor = 10,
        ...
        'doc_label' is the sentiment label of the documents. It is needed for semi-supervised document representation.
        """
        ...
        self.dbow_neighbor = dbow_neighbor
    ...
    def find_nn_idx(self,i,syn0norm,topn = 10):
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for doc in job:
            ...
            elif self.dbow_neighbor:
                tally += train_document_dbow_neighbors(self, doc.words, doctag_indexes, alpha,beta = self.beta, work = work,
                                             train_words=self.dbow_words,
                                             doctag_vectors=doctag_vectors, doctag_locks=doctag_locks)
