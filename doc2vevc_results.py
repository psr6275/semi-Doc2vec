...
docvec_list = []
for be in beta:
    f = open(data_path+'/yelp_'+year+'_doc2vec_dbow_neighbor_'+str(rr)+'_beta_'+str(be)+"_data.pickle",'rb')
...
for be_idx, be in enumerate(beta):
    ...
    nega_idx = copy.deepcopy(true_label<3)
    posi_idx = copy.deepcopy(true_label>3)
    true_label[nega_idx] = -1
    true_label[posi_idx] = 1
    train_label = np.array(docvecs['train_label'],copy = True)
    nega_idx = copy.deepcopy(np.logical_and((train_label<3),(train_label>0)))
    posi_idx = copy.deepcopy(train_label>3)
    train_label[nega_idx] = -1
    train_label[posi_idx] = 1
