...
data_path = cwd + '/data/amazon_data'
#year = '2013'
"""
...
f = open('../data/amazon_data/books_processed_unlabeled.txt','rb')
texts = pickle.load(f)
f.close()

f = open('../data/amazon_data/books_processed_unlabeled_label.txt','rb')
labels = pickle.load(f)
f.close()
...
#embed()
#print('beta = ',be)
model = Doc2Vec(sentences, size=d_size, window = 3, min_count=3, workers = 8,dm = 0, iter = 10)
    

file_name = '../results/books_doc2vec_dbow_'+str(rr)+'.doc2vec'
model.save(file_name)

doctag = list(model.docvecs.doctag_syn0)
doc2vec = {'train_label':doc_labels,'true_label':true_labels,'docvec':doctag}

doc2vec = pd.DataFrame(doc2vec)

f = open('../results/books_doc2vec_dbow_'+str(rr)+'_data.pickle','wb')
