...
try:
    from doc2vec_inner import train_document_dbow, train_document_dm, train_document_dm_concat,train_document_dbow_graph
    ...
    def train_document_dbow_graph(model, doc_words, doctag_indexes, alpha, beta, work=None,
                            train_words=False, learn_doctags=True, learn_words=True, learn_hidden=True,
                            word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None):
        """
        Update distributed bag of words model ("PV-DBOW") by training on a single document.
        ...
        for doctag_index in doctag_indexes:
            for word in doc_words:
                train_sg_pair(model, word, doctag_index, alpha, learn_vectors=learn_doctags,
                              learn_hidden=learn_hidden, context_vectors=doctag_vectors,
                              context_locks=doctag_locks)
            train_doc_graph(model,doctag_index, beta, learn_vectors=learn_doctags, learn_hidden=learn_hidden)
    
        return len(doc_words)
        
    def train_doc_graph(model,doctag_indexes, beta, learn_vectors=True, learn_hidden=True,
        ...
        nbs_indexes = [i for i in model.senti_graph[doctag_indexes,:]]    
        nbs_vectors = [model.docvecs.doctag_syn0[j] for j in nbs_indexes]
        nbs_labels = model.doc_label[nbs_indexes]
    
        lij = nbs_labels * base_label
        
        neu1e = zeros(base_doc.shape)
        l1 = base_doc # before normalize
        for nb,l in zip(nbs_vectors,lij):
            l2a = copy.deepcopy(nb) # before normalize
            nb_size = sqrt((nb**2).sum(-1))
            if l==1:
                fa = 1.0 / (1.0 + exp(-(dot(l1, l2a.T)/(base_size*nb_size))))
            else:
                fa = 1.0 / (1.0 + exp(dot(l1, l2a.T)/(base_size*nb_size)))
...
class Doc2Vec(Word2Vec):
    ...
    def __init__(self, documents=None, doc_label = None, senti_graph = None, size=300, alpha=0.025, beta = 0.05, window=8, min_count=5,
                 max_vocab_size=None, sample=0, seed=1, workers=1, min_alpha=0.0001, dbow_graph = 0,
        ...
        if word_model is not None:
            if word_model.sg!=2:
                self.layer1_size = word_model.layer1_size
            else:
                self.layer1_size = word_model.vector_size
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for doc in job:
            ...
            if self.dbow_graph:
                tally += train_document_dbow_graph(self, doc.words,doctag_indexes, alpha, self.beta, work, train_words = self.dbow_words,
