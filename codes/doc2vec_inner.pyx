...
cdef void fast_document_dbow_neighbor(
    ...
    cdef REAL_t *target_value = <REAL_t *>malloc(size*cython.sizeof(REAL_t))
    cdef REAL_t *temp = <REAL_t *>malloc(size*cython.sizeof(REAL_t))
     
    cdef REAL_t *neighbor_index = <REAL_t *>malloc(size*cython.sizeof(REAL_t))
...
def train_document_dbow_neighbor(model, doc_words, doctag_indexes, alpha, beta, work=None,
    ...
    if model.do_train % 30 == 0:
        print("update_neighbor_list! ",model.do_train)
        start_t = time.time()
        model.set_neighbor_list()
        print("elapsed time = ",time.time()-start_t)
        print("end of do_train = ",model.do_train)
    ...
    for i in range(doctag_len):
        ...
        #neighbors_list = model.most_similar everytime documents are trained the neighborhoods list are changed
