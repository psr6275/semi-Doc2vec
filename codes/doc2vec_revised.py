...
class Doc2Vec(Word2Vec):
    ...
    def set_neighbor_list(self):
        ...
        if self.nb_list is None:
            self.nb_list = zeros((m,p))
        self.docvecs.doctag_syn0norm = empty(self.docvecs.doctag_syn0.shape,dtype = REAL)
        np_divide(self.docvecs.doctag_syn0, sqrt((self.docvecs.doctag_syn0**2).sum(-1))[..., newaxis], self.docvecs.doctag_syn0norm)
        #nn = [multiprocessing.Process(target = find_nn_idx, args = (i,self.docvecs.doctag_syn0norm,)) for i in range(m)]
        nn = [find_nn_idx(i,self.docvecs.doctag_syn0norm) for i in range(m)]
